...
However, when comparing the actual time spent by the type checker, there does not seem to be a strong indication of performance degradation when comparing check times between \code{Add}, \code{Multiply} and \code{Divide}, as seen in Figure \ref{fig:check-time}. Only the \code{Root} operation does seem to have a significant negative impact on the type-checking performance. As can be seen, there is a~significant performance hit when the number of type instantiations is in the magnitude of millions. For full benchmarking results, refer to the tables found in Section \ref{appendix:performance}.
